...
This package was generated from the `zero-to-prod/sdk` template and keeps it as a git remote named `template`, so shared client code merges forward from there. Run `composer check-template` to verify the wiring and see `docs/template.md` for the runbook.
