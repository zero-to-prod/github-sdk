...
tokens across all files, the file names that carry the old identity (`src/SdkApi.php`,
`bin/sdk`, …), `composer.json`, and `sdk.json`. It deletes itself afterwards.
...
| `src/SdkApi.php`                           | `@method` docblock regenerated at the end of every run       |
...
`SdkApi` works with an empty config — tests don't need to configure a base URL. `LaravelHttpTransport` returns `Illuminate\Http\Client\Response` directly (no `ApiResult` wrapping), and Laravel auto-wildcards `Http::fake()` array keys so a bare rendered `ApiRoute` matches any host:
...
**SdkApi** — entry point. **SdkConfig** — connection settings as array with class constant keys:
...
`SdkApi::fake()` returns an in-memory transport. Queue responses (FIFO — empty queue returns 200 with empty body), call methods, then assert:
...
[$api, $fake] = SdkApi::fake(
    SdkConfigFactory::factory()->context(), // ->context() returns the resolved array
);
...
Implement `Zerotoprod\Sdk\HttpTransport`. The `@template TResponse` sets the return type when `[Options::raw => true]` is passed — so a Guzzle transport makes `raw` calls return `Psr\Http\Message\ResponseInterface`:
